<?php
	...
	function js_check_admin() {
		...
		$exists = $database->exists( 'js_employee', $check_column,  $check_for );
	...
	function js_new_option($title, $content, $employeeid) {
		$database = new Js_Dbconn();			
	...
	function js_set_option($option, $value, $employeeid) {
		$database = new Js_Dbconn();	
	...
	function js_new_options(){
	  if ( isset( $_POST['SaveSite'] ) ) {                
		...
		js_new_option('keywords', $_POST['keywords'], '1');
