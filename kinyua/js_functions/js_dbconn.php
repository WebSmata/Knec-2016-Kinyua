...
class Js_Dbconn {
    ...
    public function log_db_errors( $error, $query )
    {
        ...
        $headers .= 'Content-vehicle: text/html; charset=iso-8859-1' . "\r\n";
	...
	public function __construct()
	{
		...
		$this->link = new mysqli( JS_HOST, JS_USER, JS_PASS, JS_DB );
		$this->link->set_charset( "utf8" );
		
        if( $this->link->connect_errno )
        {
            $this->log_db_errors( "Connect failed", $this->link->connect_error );
			die(JS_I_TOP.'START SETTING UP THINGS'.JS_I_TOP_A.'<form action="'.js_database_setup().'" method="post">
                        <h5>Set a few options to start you off... on: '.JS_SITEURL.'</h5><table>
             ...
             <tr><td>Database Password:</td><td><input type="password" name="password" autocomplete="off" min="8"></td></tr> 
              ...
              <center><input type="submit" class="submit_this" name="DatabaseSetup" value="Save Options"></center><br></form>'.JS_I_BOT);
            exit();
        }
	}
    ...
    public function js_table_exists( $name )
    {
    ...
    public function js_table_exists_create( $table,  $variables = array() ) {
    ...
    public function js_num_rows( $query )
    {
    ...
    public function exists( $table = '', $check_val = '', $params = array() )
    {
    	...
    	$number = $this->js_num_rows( $rs_check );
        if( $number === 0 )
        {
            return false;
        }
    ...
    public function js_insert( $table, $variables = array() )
    {
    ...
    public function js_update( $table, $variables = array(), $where = array(), $limit = '' )
    {
